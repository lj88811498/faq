...
Role.openAddRole = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/sysRole/role_add'
    });
...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/sysRole/role_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/sysRole/remove", function () {
                Feng.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/sysRole/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
$(function () {
    ...
    var table = new BSTableOld(Role.id, "/sysRole/list", defaultColunms);
