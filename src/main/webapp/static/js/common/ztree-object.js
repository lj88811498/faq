    ...
    $ZTreeCC.prototype = {
        ...
        initSetting : function() {
            var settings = {
                ...
                data : {simpleData : {enable : true,name:"organizationName"}},
                callback : {
                    onClick : this.onClick,
                    onDblClick:this.ondblclick,
        ...
        loadNodes : function() {
            ...
            var newZnodes = [];
            var ajax = new $ax(Feng.ctxPath + this.url, function(data) {
                zNodes = data.page.records;
                for(var i = 0; i < zNodes.length; i++){
                    newZnodes.push({
						id:zNodes[i].organizationId,
						pId:zNodes[i].organizationPid,
						name:zNodes[i].organizationName,
                        organizationStatus:zNodes[i].organizationStatus,
                        organizationPids:zNodes[i].organizationPids
					});
				}
            }, function(data) {
                Feng.error("加载ztree信息失败!");
