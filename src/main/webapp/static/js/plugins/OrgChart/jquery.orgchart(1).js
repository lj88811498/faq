...
(function(factory) {
  ...
  function showParent($node) {
    ...
    var $temp = $node.closest('table').closest('tr').siblings().removeClass('hidden');
     // $node.closest('table').closest('tr').children("td").children("a").css("width","280px");
  ...
  function switchVerticalArrow($arrow) {
    $arrow.toggleClass('fa-minus').toggleClass('fa-plus');
  }
  ...
  function createNode(nodeData, level, opts) {
    ...
    // construct the content of node
         ...
         if(nodeData.id == "1" || nodeData.id == "2"){
             styleWidth = "280px"
         }
         else{
             styleWidth = "110px";
      ...
      if (typeof opts.nodeContent !== 'undefined' ){
          nodeData[opts.nodeContent] = nodeData[opts.nodeContent];
      }
      else{
          nodeData[opts.nodeContent] = "";
      }
    var $nodeDiv = $('<a style="width:'+styleWidth+'" ' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>')
    ...
    // append 4 direction arrowsconsole.log(nodeData.id);
    ...
    if(Number(flags.substr(2,1))) {
        var $bottomEdge = $nodeDiv.children('.bottomEdge');
        var flag = false;
        if ($bottomEdge.length) {
            if(nodeData.id != "1" && nodeData.id != "2"){
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-plus" style="left:43px;"></i>')
                    .children('.title').prepend('<i class="fa '+ opts.parentNodeSymbol + ' symbol"></i>');
            }
            else{
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-plus"></i>')
                    .children('.title').prepend('<i class="fa '+ opts.parentNodeSymbol + ' symbol"></i>');
            }
        }
        else{
            if(nodeData.id != "1" && nodeData.id != "2"){
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-minus" style="left:43px;"></i>')
                    .children('.title').prepend('<i class="fa '+ opts.parentNodeSymbol + ' symbol"></i>');
            }
            else{
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-minus"></i>')
                    .children('.title').prepend('<i class="fa '+ opts.parentNodeSymbol + ' symbol"></i>');
            }
           /* $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-minus"></i>')
                .children('.title').prepend('<i class="fa '+ opts.parentNodeSymbol + ' symbol"></i>');*/
        ...
        if(!getNodeState($nodeDiv, 'children').visible){
            if(nodeData.id != "1" && nodeData.id != "2") {
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa fa-plus" style="left:43px;"></i>')
                    .children('.title').prepend('<i class="fa ' + opts.parentNodeSymbol + ' symbol"></i>');
            }
    ...
    $nodeDiv.on('mouseenter mouseleave', function(event) {
      ...
      if (event.type === 'mouseenter') {
        ...
        if ($bottomEdge.length) {
          ...
          $bottomEdge.toggleClass('fa-plus', !flag).toggleClass('fa-minus', flag);
        }
        if ($leftEdge.length) {
          switchHorizontalArrow($node);
        }
      } else {
         // $node.children('.edge').removeClass('fa-chevron-up fa-chevron-down fa-chevron-right fa-chevron-left fa-plus fa-minus');
    ...
    $nodeDiv.on('click', '.bottomEdge', function(event) {
        var $that = $(this);
        var $node = $that.parent();
        var $nodeSiblings = $node.closest("table").parent().siblings().children().children().children("tr.feng");
        var $nodeSiblingsFa = $node.closest("table").parent().siblings().children().children().children("tr.feng").children().children(".node").children(".bottomEdge");
        var $nodeSiblings1 = $node.closest("table").parent().siblings().children().children().children("tr.lines");
        var $nodeSiblings2 = $node.closest("table").parent().siblings().children().children().children("tr.nodes");
        var childrenState = getNodeState($node, 'children');
        var $nodeSiblingsState = getNodeState($nodeSiblings, 'children');
      if (childrenState.exist) {
        ...
        if (childrenState.visible) {
          hideDescendants($node);
            $nodeSiblings1.addClass("hidden");
            $nodeSiblings2.addClass("hidden");
            $nodeSiblingsFa.removeClass("fa-minus").addClass("fa-plus");
  ...
  function buildHierarchy ($appendTo, nodeData, level, opts, callback) {
    ...
    if (Object.keys(nodeData).length > 1) { // if nodeData has nested structure
      ...
      .done(function($nodeDiv) {
        $table.append($nodeDiv.wrap('<tr class="feng"><td' + (hasChildren ? ' colspan="' + $childNodes.length * 2 + '"' : '') + '></td></tr>').closest('tr'));
        if (callback) {
          callback();
        }
      })
  ...
  function buildParentNode($currentRoot, nodeData, opts, callback) {
    ...
    $.when(createNode(nodeData, 0, opts || $currentRoot.closest('.orgchart').data('options')))
      .done(function($nodeDiv) {
        ...
        oc.prepend($table)
          .children('table:first').append('<tr class="nodes"><td colspan="2" style="width:280px;"></td></tr>')
