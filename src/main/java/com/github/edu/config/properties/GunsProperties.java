...
import static com.github.edu.core.util.ToolUtil.getTempPath;
import static com.github.edu.core.util.ToolUtil.isEmpty;
...
public class GunsProperties {
    ...
    @Value("${guns.file-upload-pathlib}")
    private String fileUploadPathLib;
...
//            }
    ...
    public String getFileUploadPathLib() {
        //fileUploadPath = "W:\\fx";
        //如果没有写文件上传路径,保存到临时目录
        if (isEmpty(fileUploadPathLib)) {
            return getTempPath();
        } else {
            //判断有没有结尾符,没有得加上
//            if (!fileUploadPath.endsWith(File.separator)) {
...
//            }
            //判断目录存不存在,不存在得加上
            if (!haveCreatePath) {
                File file = new File(fileUploadPathLib);
                file.mkdirs();
                haveCreatePath = true;
            }
            return fileUploadPathLib;
        }
    }
//    public String getFileUploadPathLib() {
//        return fileUploadPathLib;
//    }
    ...
    public void setFileUploadPathLib(String fileUploadPathLib) {
        this.fileUploadPathLib = fileUploadPathLib;
    }
